...
# OAuth callbackでセッションCookieに依存しすぎないよう、state自体を署名して検証する。
app.config.update(
...
def convert_cookies_to_link(raw_data):
    ...
    try:
        ...
        if isinstance(data, list):
            for item in data:
                ...
                if name == "NetflixId":
                    ...
                    secure_netflix_id = value
        elif isinstance(data, dict):
            netflix_id = data.get("NetflixId", "")
            secure_netflix_id = data.get("SecureNetflixId", "")
        ...
        for line in lines:
            ...
            if len(parts) >= 7:
                ...
                if "NetflixId=" in line and "Secure" not in line:
                    netflix_id = line.split("NetflixId=")[1].split(";")[0].strip()
                elif "SecureNetflixId=" in line:
                    secure_netflix_id = line.split("SecureNetflixId=")[1].split(";")[0].strip()
    ...
    if netflix_id:
        ...
        generated_link = f"https://www.netflix.com/?cookie_payload={urllib.parse.quote(cookie_header)}"
        return generated_link, cookie_header
    
    return None, None

# --- NetflixのCookieが有効かチェックする関数 ---
def check_netflix_live(cookie_header):
...
def login():
    ...
    # セッションCookieだけにstateを保存せず、署名付きstateとしてDiscordへ渡す。
...
def callback():
    ...
    if not state or not code:
        return 'Discord OAuth認証に必要な情報がありません。最初からログインし直してください。', 400

    try:
        # stateは10分以内に発行された署名済みのものだけ受け付ける。
        oauth_state.loads(state, max_age=600)
    except SignatureExpired:
        return 'Discord OAuthの認証が期限切れです。最初からログインし直してください。', 400
    except BadSignature:
        return 'Discord OAuthのstate検証に失敗しました。最初からログインし直してください。', 400
...
def claim():
    ...
    with db() as c:
        ...
        while True:
            ...
            if "cookie_payload=" in raw_url:
                parsed_payload = raw_url.split("cookie_payload=")[1]
                cookie_header = urllib.parse.unquote(parsed_payload)
            else:
                cookie_header = raw_url

            if check_netflix_live(cookie_header):
                c.execute('UPDATE inventory SET used=1, used_by=? WHERE id=? AND used=0', (u['id'], row['id']))
                c.execute('INSERT INTO claims(discord_id, url) VALUES(?, ?)', (u['id'], raw_url))
                target_url = raw_url
                break
            else:
...
def admin():
    ...
    if request.method == 'POST':
        ...
        with db() as c:
            ...
            for block in blocks:
                link, _ = convert_cookies_to_link(block)
                if link:
                    try:
    ...
    <div class="container">
        <div class="card">
            ...
            <form method=post enctype=multipart/form-data>
                <label style="font-size:0.85rem; color:#9ba1a6;">Cookieチェッカー結果を貼り付け:</label>
                <textarea name=urls rows=6 placeholder="ここに貼り付け..."></textarea>

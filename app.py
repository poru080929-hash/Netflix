import os, sqlite3, secrets, requests, urllib.parse, re, html
...
def claim():
    ...
    with db() as c:
        ...
        # --- 最終修正：先頭の「ct=」も完全に削り、ブラウザで動作させるための処理 ---
        match = re.search(r'NetflixId["\'\s:=]+([^\s;"\']+)', target_url)
        if match:
            val = match.group(1)
            # 1. まずデコードして、邪魔な文字を削れる状態にする
            val = urllib.parse.unquote(val)
            # 2. 「v=3&ct=」や「ct=」など、先頭の不要な部分を完全に削る
            val = re.sub(r'^(?:v=\d+&)?ct=', '', val)
            # 3. ブラウザでアクセスした際に「+」が「空白」に壊れるのを防ぐため、必ず再エンコードする
            safe_val = urllib.parse.quote(val)
            
            generated_link = f"https://netflix.com/unsupported?nftoken={safe_val}"
        else:

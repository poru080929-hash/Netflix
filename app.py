...
def admin():
    ...
    if request.method == 'POST':
        ...
        with db() as c:
            # 汎用的なブロック分割: 空行、または「===」や「---」などの区切り線で分割する
            blocks = re.split(r'\n\s*\n|={3,}|-{3,}', raw_text)
            for block in blocks:
                block = block.strip()
                # ブロック内に NetflixId や .netflix.com、あるいは nftoken が含まれているかを判定
                if block and ('NetflixId' in block or '.netflix.com' in block or 'nftoken=' in block):
                    try:
                        c.execute('INSERT INTO inventory(url) VALUES(?)', (block,))

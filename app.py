...
def admin():
    ...
    if request.method == 'POST':
        ...
        with db() as c:
            # どんなチェッカーの出力であっても、中から nftoken を自動で全て探し出す
            # 1. すでに https://netflix.com/unsupported?nftoken=... の形になっているものを探す
            complete_urls = re.findall(r'https://[^\s]+\?nftoken=[A-Za-z0-9\-_%+=]+', raw_text)
            
            # 2. テキスト内の `nftoken=` や `nftoken%3D` の後ろのトークン文字列をすべて抽出する
            tokens = re.findall(r'nftoken[=,%3D]([A-Za-z0-9\-_%+=]+)', raw_text)
            
            found_links = set(complete_urls)
            for t in tokens:
                decoded_t = urllib.parse.unquote(t)
                found_links.add(f"https://netflix.com/unsupported?nftoken={decoded_t}")
            
            # 見つかったリンクを1つずつ在庫としてデータベースに登録
    ...
    <div class="container">
        <div class="card">
            ...
            <form method=post enctype=multipart/form-data>
                <label style="font-size:0.85rem; color:#9ba1a6;">チェッカーの出力を貼り付け (自動でリンクに変換されます):</label>

...
def get_redirect_uri():
    # DISCORD_REDIRECT_URI が指定されていない場合は現在のドメインから動的生成
    if REDIRECT:
        return REDIRECT
...
def login():
    if not CID:
        abort(500, 'DISCORD_CLIENT_ID が設定されていません。')
...
def claim():
    ...
    # http(s)://から始まる文字列の場合は<a>タグリンクとしてクリック可能にする
    is_link = target_url.startswith(('http://', 'https://'))
    ...
    <div class="container">
        <div class="card" style="text-align:center;">
            ...
            <p>アカウントの生成に成功しました。</p>
            <div class="link-box" id="targetLink">
                {% if is_link %}
                    <a href="{{target_url}}" target="_blank" rel="noopener noreferrer">{{target_url}}</a>
                {% else %}
                    {{target_url}}
                {% endif %}
            </div>
    ...
    <script>
    function copyLink() {
        const text = document.getElementById('targetLink').innerText.trim();
    ...
    '''
    return render_template_string(html, target_url=target_url, is_link=is_link)

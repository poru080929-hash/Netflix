...
def claim():
    ...
    with db() as c:
        ...
        # うまく抽出できなかった場合の保険（元の生データをそのまま出す）
...
def admin():
    ...
    for r in rows:
        ...
        first_line = "データ"
        for line in r['url'].splitlines():
            line_str = line.strip()
            if line_str and not line_str.startswith('.'):
                first_line = line_str[:35] + "..."
                break
        ...
        <div style="background: #0b0b0e; border: 1px solid #22222c; padding: 12px; border-radius: 10px; margin-bottom: 10px;">
            ...
            <details>
                <summary style="font-size:0.75rem; color:#9ba1a6;">▼ 内容を表示/隠す ({first_line})</summary>
                <textarea readonly style="width:100%; height:120px; resize:vertical; margin-top:8px; background:#141418; border:1px solid #22222c; color:#f3f3f5; padding:8px; border-radius:8px;">{safe_val}</textarea>
            </details>
        </div>
        '''

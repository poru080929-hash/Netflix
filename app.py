...
def admin():
    ...
    if request.method == 'POST':
        ...
        with db() as c:
            # 完全に「NETFLIX ACCOUNT DETAILS」を基準に1アカウントずつ正確に分割する
            # テキスト内に複数のアカウントが含まれていても、ここで綺麗に1つずつ分かれます
            parts = raw_text.split("NETFLIX ACCOUNT DETAILS")
            for part in parts:
                part = part.strip()
                if not part:
                    continue
                # キーワードを復元して1つのアカウントブロックを作る
                account_data = "NETFLIX ACCOUNT DETAILS\n" + part
                
                # 最低限 NetflixId または .netflix.com が含まれている有効なデータのみ登録
                if "NetflixId" in account_data or ".netflix.com" in account_data:
                    try:
                        c.execute('INSERT INTO inventory(url) VALUES(?)', (account_data,))
    ...
    for r in rows:
        ...
        rows_html += f"<div style='background:#0b0b0e; padding:10px; border-radius:8px; margin-bottom:10px;'><p style='font-size:0.75rem; word-break:break-all; margin:0;'><b>ID: {r['id']}</b> | {status_badge}<br><br>{safe_url}</p></div><hr>"
    ...
    <div class="container">
        <div class="card">
            ...
            <form method=post enctype=multipart/form-data>
                ...
                <textarea name=urls rows=8 placeholder="NETFLIX ACCOUNT DETAILS..."></textarea>

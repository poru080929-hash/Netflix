...
def admin():
    ...
    if request.method == 'POST':
        ...
        with db() as c:
            # 正規表現で text 全体から nftoken= の後ろに続く英数字・記号の塊（トークン）をすべて綺麗に抽出する
            # 例: nftoken=ABC123xyz... の部分だけを正確に抜き出す
            tokens = re.findall(r'nftoken=([a-zA-Z0-9\-_.~%]+)', raw_text)
            for token in tokens:
                token = token.strip()
                if token and len(token) > 10:  # 十分な長さがある有効なトークンのみ
                    link = f"https://www.netflix.com/unsupported?nftoken={token}"
                    try:

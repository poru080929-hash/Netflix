...
def convert_cookies_to_link(raw_data):
    ...
    # すでに正しい unsupported?nftoken= の形であればそのまま返す
    if "unsupported?nftoken=" in raw_data:
        parts = raw_data.split("nftoken=")
        if len(parts) > 1:
            token = parts[1].split()[0].strip()
            return f"https://www.netflix.com/unsupported?nftoken={token}", token
    # トークン部分（またはURL全体の文字列）から綺麗に組み立てる
    if "nftoken=" in raw_data:
        token = raw_data.split("nftoken=")[1].split()[0].strip()
        return f"https://www.netflix.com/unsupported?nftoken={token}", token
    ...
    if request.method == 'POST':
        ...
        for uploaded_file in uploaded_files:
            if uploaded_file and uploaded_file.filename:
                file_texts.append(uploaded_file.read().decode('utf-8', errors='ignore'))
        ...
        with db() as c:
            ...
            for line in lines:
                ...
                if not line_str or line_str.startswith("#"):
                    continue
                link, _ = convert_cookies_to_link(line_str)
                if link:
                    try:
